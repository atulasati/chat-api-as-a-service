...
def searchUserInGroup(request):
    ...
    try:
        ...
        if(request.query_params.__contains__('search')):
            ...
            room = Room.objects.get(id = request.query_params['room_id'])
            print("=============== this is inside searchuseringroup",room)
...
def createGroup(request):
    ...
    try:
        ...
        return JsonResponse( {'status':"ok",'room':RoomSerializer(room).data }, status=201)
    except Exception as e:
        print("Error",e)
        return JsonResponse({'status':"false","error":f'{e}'}, safe=False, status=400)
